    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="relative p-6 md:p-8 rounded-3xl overflow-hidden border border-gray-100 bg-white shadow-soft">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 relative z-10">
          <div>
            <h1 className="text-2xl font-black text-dark-900 tracking-tight">Team Management</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8 pt-6 border-t border-gray-100">
          ...
          <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100 flex items-center gap-4">
            ...
            <div>
              <p className="text-[12px] text-dark-400 font-bold uppercase tracking-wider">Total Members</p>
              <h3 className="text-xl font-black text-dark-900 mt-0.5">{totalMembers}</h3>
            </div>
          </div>
          ...
          <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100 flex items-center gap-4">
            ...
            <div>
              <p className="text-[12px] text-dark-400 font-bold uppercase tracking-wider">Active Users</p>
              <h3 className="text-xl font-black text-dark-900 mt-0.5">{activeMembers}</h3>
            </div>
          </div>
          ...
          <div className="p-4 bg-gray-50/50 rounded-2xl border border-gray-100 flex items-center gap-4">
            ...
            <div>
              <p className="text-[12px] text-dark-400 font-bold uppercase tracking-wider">Pending Invites</p>
              <h3 className="text-xl font-black text-dark-900 mt-0.5">{pendingMembers}</h3>
            </div>
          </div>

          <div className="p-4 bg-[#F8F5FE] rounded-2xl border border-[#E5DCFC] flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#EDE5FC] text-[#7C3AED] flex items-center justify-center flex-shrink-0">
              <Shield className="w-4.5 h-4.5" />
            </div>
            <p className="text-[12px] text-[#3B3054] font-semibold leading-relaxed">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3 bg-white rounded-3xl border border-gray-100 shadow-soft">
          ...
          <span className="text-[15px] text-dark-400 font-semibold">Loading team members...</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {members.map((m) => {
            ...
            let avatarClass = 'bg-[#EDE5FC] text-[#7C3AED] border-[#E5DCFC]';
            let rolePillClass = 'bg-[#EDE5FC] text-[#7C3AED] border-[#E5DCFC]';
            if (m.role === 'Accountant') {
              avatarClass = 'bg-[#ecfdf5] text-[#16A34A] border-green-100';
              rolePillClass = 'bg-[#ecfdf5] text-[#16A34A] border-green-100';
            } else if (m.role === 'Manager') {
              avatarClass = 'bg-blue-50 text-blue-600 border-blue-100';
              rolePillClass = 'bg-blue-50 text-blue-600 border-blue-100';
            } else if (m.role === 'Employee') {
              avatarClass = 'bg-slate-50 text-slate-600 border-slate-200';
              rolePillClass = 'bg-slate-50 text-slate-600 border-slate-200';
            }
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold text-dark-900 text-[15px] leading-snug">{m.name}</h4>
                        <span className="text-[12px] text-dark-400 font-bold flex items-center gap-1 mt-0.5">
                  ...
                  <div className="mt-5 flex items-center gap-2 flex-wrap">
                    ...
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold border ${
                      isActive
                        ? 'bg-[#ecfdf5] text-success border-green-150'
                  ...
                  {/* Description Box */}
                  <div className="mt-4 p-3 bg-gray-50/50 rounded-xl border border-gray-100 flex items-start gap-2">
                    <Info className="w-4 h-4 text-dark-400 flex-shrink-0 mt-0.5" />
                    <p className="text-[12px] text-dark-400 font-semibold leading-relaxed">
                      {getRoleDesc(m.role)}
                    </p>
                  </div>
                </div>
                ...
                {/* Joining date info */}
                {m.created_at && (
                  <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between text-[11px] font-bold text-dark-400">
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-[12px] font-bold text-dark-400 uppercase tracking-wider">Full Name *</label>
                  <input
                    type="text"
                    value={name}
                ...
                <div className="space-y-1.5">
                  <label className="text-[12px] font-bold text-dark-400 uppercase tracking-wider">Email Address *</label>
                  <input
                    type="email"
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="e.g. ayeaye@company.com"
                    className="w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-[15px] placeholder:text-dark-400 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500/40 font-medium disabled:opacity-50"
                    required
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-[12px] font-bold text-dark-400 uppercase tracking-wider">Role Access *</label>
                    <select
                      value={role}
                      onChange={(e) => setRole(e.target.value as any)}
                      className="w-full px-4 py-2.5 rounded-xl bg-gray-50 border border-gray-200 text-[15px] focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500/40 font-medium"
                    >
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[12px] font-bold text-dark-400 uppercase tracking-wider">Invite Status *</label>
                    <select
                      value={status}
                ...
                <div className="p-3 bg-[#F8F5FE] border border-[#E5DCFC] rounded-xl text-xs text-[#3B3054] flex gap-2">
                  <Shield className="w-4.5 h-4.5 text-[#7C3AED] flex-shrink-0 mt-0.5" />
                  <div>
